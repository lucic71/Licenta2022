...
\lm{This is not the only purpose of a chassis. Its main purpose is to integrate
all the testing elements. In IxExplorer you can view, for example, multiple
chassis whose ports are interconnected, in fact this is quite a common testing
setup. The advantage of this (chassis, multiple-chassis, AKA ``chassis chain'')
view is that it integrates the entire test setup in a single place. You can for
example correlate traffic generated from Chasssis 1, Card 1, Port 1 to traffic
received by Chassis 2, Card 1, Port 5 by viewing them in the same place. It also
scales for big setups. \\\\ LL: I think that my description is enough for the scope of the thesis.
}
...
initialization of our Card.~\lm{This could be better focused, by first mentioning that we had problems with InterfaceManager initialization, then saying that these required an iterative testing/searching/rebuilding/redeployment process.}
