...
\lm{You should also restate here what precisely the chapter is discussing/presenting.}
...
seldom an easy task because~\lm{I would add here: in general} it involves a good amount of code refactoring and
rewriting. It can be avoided, however, if~\lm{in particular} the original design has portability
incorporated by using constructs as multi-platform libraries, modular code or
standard compiler behaviors.~\lm{I'm nitpicking here, but I think you're going over these constructs much too fast; are these just examples? is there an exhaustive list somewhere (in Mooney's paper, maybe)? portability design seems to be important, so it might be worth going into a bit more detail.}
...
hardware attached to these processors makes~\lm{may make?} porting difficult as the software
...
porting tasks, let us describe the porting costs associated with the process.~\lm{You can leave out this paragraph entirely.}

Porting costs~\lm{and more generally, software development costs} are measured in man-hours~\cite{tanaka, hakuta}. While the costs are
...
0.~\lm{Might be worth provide a particular example of an $S_i$.}
